...
`CLAUDE.md`의 `## 진행 상황` 섹션은 **작품별로 독립된 테이블**로 관리된다. 각 작품은 `### 작품명: {작품명}` 헤더와 Phase 상태 테이블을 가진다.

- **각 Phase 시작 시**: `CLAUDE.md`에서 현재 작품의 `### 작품명: {작품명}` 섹션을 찾아 해당 Phase 상태만 `🔄 진행 중`으로 업데이트 (다른 작품 섹션 수정 금지)
- **각 Phase 완료 시**: `CLAUDE.md`에서 현재 작품의 `### 작품명: {작품명}` 섹션을 찾아 해당 Phase 상태만 `✅ 완료`로 업데이트 (다른 작품 섹션 수정 금지)
- **진행 상황 표시 시**: `CLAUDE.md`를 Read 도구로 읽어 현재 작품의 `### 작품명: {작품명}` 섹션만 출력
- **신규 작품 추가**: Phase 0 완료 후, `CLAUDE.md`의 `## 진행 상황` 섹션 맨 아래에 아래 블록을 `---` 구분선과 함께 추가
...
4. 기존 `output/{작품명}/` 디렉토리가 있으면 덮어쓰기(새로 시작) 또는 재개 여부 확인
   - **새로 시작(덮어쓰기)**: 기존 output 파일 삭제 후 → 5번으로 이동
   - **재개**: 5번 건너뛰고 → `## 재개` 섹션으로 이동하여 output 파일 기반으로 CLAUDE.md 동기화 후 재개
5. (새로 시작 시만 실행) `CLAUDE.md`의 `## 진행 상황` 섹션에 현재 작품 `### 작품명: {작품명}` 블록이 없으면 맨 아래에 신규 추가. 이미 있으면 해당 섹션의 전체 Phase 상태를 `⬜ 대기`로 초기화. **다른 작품 섹션은 절대 수정하지 않음**
...
- **MUST DO**:
  ...
  - `overview-concept.png`: 제안서 1.작품개요 + 2.기획의도의 핵심 내용을 **고객(교사·학부모·기관담당자) 관점의 인포그래픽으로 도식화**. 도형·아이콘·섹션 레이아웃을 활용하여 한 장에 구조적으로 배치. **한글 텍스트 레이블 포함** — 고객이 얻는 가치와 경험 중심으로 작성 (예: "아이가 직접 결정해요", "효과를 숫자로 확인", "화재·교통·지진 한 번에", "탐정 수료증 수여"). **공급자 관점 메시지 금지** — 재계약률, 투자 회수, BEP, 수익 시나리오 등 내부 경영 지표는 절대 포함하지 않음.
...
모든 산출물 파일 존재 확인 후 `CLAUDE.md`를 Read 도구로 읽어 `## 진행 상황` 섹션을 포함하여 사용자에게 최종 보고:
```
✅ 공연 기획 제안서 자동 생성 완료
...
`CLAUDE.md`의 `## 진행 상황` 섹션 전체 Phase 상태가 `✅ 완료`로 표시되어 있는지 확인.
...
**재개 시 필수**: 파일 존재 여부로 완료 판단 후, CLAUDE.md의 해당 Phase 상태를 즉시 동기화:
- 파일 있음 → `CLAUDE.md` 해당 Phase 상태를 `✅ 완료`로 업데이트
- 파일 없음 → `CLAUDE.md` 해당 Phase 상태를 `⬜ 대기`로 유지 (변경 없음)
- 동기화 완료 후 `CLAUDE.md`를 Read 도구로 읽어 진행 상황 표시
